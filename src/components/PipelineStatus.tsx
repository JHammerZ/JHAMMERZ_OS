...
export const PipelineStatus: React.FC = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          console.warn("Pipeline status response not JSON:", contentType);
        }
      }
    } catch (error) {
  ...
  // Status-specific color definitions matching sovereign themes
  ...
  return (
    <div className="w-full bg-sovereign-card brutalist-border p-4 relative overflow-hidden group">
      {/* Dynamic scanline element for running processes */}
      {isRunning && (
        <span className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-sovereign-neon to-transparent animate-pulse opacity-40" />
      )}

      {/* Top Section: List of Available Channels / Pipelines */}
      <div className="flex flex-wrap items-center gap-2 border-b border-sovereign-line pb-3 mb-4">
        ...
        {pipelinesList.map((p) => {
          ...
          return (
            <button
              key={p.id}
              onClick={() => setActiveStateId(p.id)}
              className={`px-3 py-1.5 font-mono text-[9px] uppercase transition-all duration-300 flex items-center gap-2 border ${
                isCurrent 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        {/* Connection status and Active Workflow Title */}
        ...
        {/* Console logs toggle & manual trigger */}
        <div className="flex items-center gap-2 self-end md:self-center">
          <button
            onClick={() => setShowLogs(!showLogs)}
            className="px-3 py-1.5 text-gray-400 hover:text-white border border-gray-800 hover:border-white transition-colors flex items-center gap-1.5 font-mono text-[10px] uppercase"
          >
          ...
          <button
            ...
            className="px-3 py-1.5 bg-sovereign-neon/10 hover:bg-sovereign-neon text-sovereign-neon hover:text-black border border-sovereign-neon/30 hover:border-sovereign-neon disabled:opacity-40 disabled:pointer-events-none transition-all flex items-center gap-1.5 font-mono text-[10px] uppercase font-bold"
          >
      ...
      {/* Log console panel with exit transition */}
      <AnimatePresence>
        {showLogs && (
          <motion.div
            ...
            className="overflow-hidden mt-4 pt-4 border-t border-sovereign-line"
          >
