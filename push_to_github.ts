...
const REPO = 'JHAMMERZ_OS';

// Grab token from env
const token = process.env.GITHUB_TOKEN || process.env.AURELIUS_SOVEREIGN_TOKEN || process.env.aurelius_sovereign_token;

if (!token) {
  console.error("❌ ERROR: No GITHUB_TOKEN or AURELIUS_SOVEREIGN_TOKEN found in your environment variables.");
  console.log("Please make sure you have added GITHUB_TOKEN or AURELIUS_SOVEREIGN_TOKEN in your platform settings.");
  process.exit(1);
}
...
async function run() {
  ...
  console.log(`🔍 Querying remote tree structure from JHammerZ/JHAMMERZ_OS...`);
